...
const ManageDamage = () => {
  ...
  const filteredHistory = damageHistory.filter((record) => {
    const startDateMatch =
      !filters.startDate || new Date(record.timestamp) >= filters.startDate;
    const endDateMatch =
      !filters.endDate || new Date(record.timestamp) <= filters.endDate;
    // Fix warehouse filter: support both warehouse and warehouseId
    const warehouseId =
      record.lotId?.warehouse?._id || record.lotId?.warehouseId?._id;
    const warehouseMatch =
      filters.warehouse === "all" ||
      (warehouseId && warehouseId.toString() === filters.warehouse);
    const userMatch =
      !filters.user ||
    ...
    try {
      ...
      if (!isAdmin && user.warehouse) {
        const defaultWarehouseId =
          typeof user.warehouse === "object" && user.warehouse._id
  ...
  const fetchProducts = async (warehouseId) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${API_BASE_URL}/api/products?warehouse=${warehouseId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      ...
      toast.error(
        error.response?.data?.message ||
          "Failed to load products. Please ensure the warehouse has associated products.",
      );
      setProducts([]);
    } finally {
      setIsLoading(false);
  ...
  const fetchLots = async (productId) => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${API_BASE_URL}/api/lots?productId=${productId}&warehouse=${selectedWarehouse}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const fetchDamageHistory = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${API_BASE_URL}/api/manage-damage/history`,
        {
          headers: { Authorization: `Bearer ${token}` },
          params,
        },
      ...
      setDamageHistory(
        data.map((record) => ({
          ...
          lotId: {
            ...record.lotId,
            productId: record.lotId?.productId || {
              name: "N/A",
              productCode: "N/A",
            },
  ...
  const visibleWarehouses = isAdmin
    ? warehouses
    : warehouses.filter(
        (w) => w._id && w._id.toString() === assignedWarehouseId,
      );
                    ...
                    setDamageHistory(
                      data.map((record) => ({
                        ...
                        lotId: {
                          ...
                          warehouseId: record.lotId?.warehouse || {
                            name: "N/A",
                          },
                        },
                      })),
                      ...
                      <TableRow key={record._id}>
                        ...
                        <TableCell>
                          <div className="font-medium">
                            {record.lotId?.productId?.name || "N/A"}
                          </div>
